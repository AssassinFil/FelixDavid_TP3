	...
	public function add() {
		if ($this->request->is('post')) {
			...
			if ($this->Policy->save($this->request->data)) {
				$this->Session->setFlash(__('The policy has been saved.'), array ('class' => 'alert alert-success'));
				return $this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__('The policy could not be saved. Please, try again.'), array ('class' => 'alert alert-danger'));
			}
		}
		$agents = $this->Policy->Agent->find('list');
		$clients = $this->Policy->Client->find('list');
		$clients = $this->Policy->Client->find('list');
		$this->set(compact('agents', 'clients', 'clients'));
	}
